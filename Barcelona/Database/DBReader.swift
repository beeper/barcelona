...
public struct DBReader {
    ...
    init(pool: DatabasePool = databasePool) {
        self.pool = pool
    }
    ...
    func resolveSenderID(forMessage message: RawMessage) -> Promise<String?, Error> {
        resolveSenderIDs(forMessages: [message]).map(\.[message.ROWID])
    }
    ...
    func resolveSenderIDs(forMessages messages: [RawMessage]) -> Promise<[Int64: String], Error> {
            ...
            pool.asyncRead {
                ...
                case .success(let db):
                    do {
                        ...
                        resolve(messages.reduce(into: [Int64: String]()) { (ledger, message) in
    ...
    /// - Returns: NIO future
    func insert(fileTransfer: IMFileTransfer, path: String) -> Promise<Void, Error> {
        Promise { resolve, reject in
...
extension DBReader {
    ...
    func handleTimestampRecords(forChatIdentifiers chatIDs: [String]) -> Promise<[HandleTimestampRecord], Error> {
        Promise { resolve, reject in
...
extension DBReader {
    static let shared: DBReader = DBReader(pool: databasePool)
}
