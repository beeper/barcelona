...
public class ERTimeSortedParticipantsManager {
    ...
    public func bootstrap(chat: IMChat) -> EventLoopFuture<Void> {
        bootstrap(chats: [chat])
    }
    
    public func bootstrap(chats: [IMChat]) -> EventLoopFuture<Void> {
        DBReader.shared.handleTimestampRecords(forChatIdentifiers: chats.compactMap { $0.chatIdentifier }).map { records in
