...
extension DBReader {
    func queryMessages(withParameters params: MessageQueryParameters) -> EventLoopFuture<[Message]> {
        ...
        var chatRowIDQuery: EventLoopFuture<[Int64]>!
        
        if let chatIdentifiers = params.chats {
            chatRowIDQuery = self.rowIDs(forIdentifiers: chatIdentifiers).map {
                Array($0.values).flatMap { $0 }
            }
        } else {
            chatRowIDQuery = self.eventLoop.makeSucceededFuture([])
        }
        ...
        return chatRowIDQuery.flatMap { ROWIDs -> EventLoopFuture<ArraySlice<RawMessage>> in
            let promise = self.eventLoop.makePromise(of: ArraySlice<RawMessage>.self)
            ...
            databasePool.asyncRead { result in
                ...
                promise.succeed(messagesFromMe.prefix(limit))
            }
            ...
            return self.chatIdentifiers(forMessageRowIDs: messageROWIDs).flatMap { chatIdentifiers in
                ...
                return EventLoopFuture<[Message]>.whenAllSucceed(masterMap.map {
    ...
    func attachments(matchingParameters parameters: AttachmentSearchParameters) -> EventLoopFuture<[InternalAttachment]> {
        parameters.chatROWIDs().flatMap { ROWIDs in
            let promise = self.eventLoop.makePromise(of: [InternalAttachment].self)
...
""")
    ...
    func messages(matching text: String, limit: Int) -> EventLoopFuture<[Message]> {
