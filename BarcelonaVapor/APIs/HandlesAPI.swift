...
extension Array where Element == IMServiceStyle {
    func idStatuses(forHandles handles: [String], on eventLoop: EventLoop) -> EventLoopFuture<[IDSStatuses]> {
        ...
        }, on: eventLoop).map { availabilities in
...
/** Manages handles */
internal func bindHandlesAPI(_ app: RoutesBuilder) {
    ...
    handles.get("ids") { req -> EventLoopFuture<BulkIDSStatuses> in
        ...
        return IMServiceStyle.allCases.idStatuses(forHandles: handleIDs, on: req.eventLoop).map {
