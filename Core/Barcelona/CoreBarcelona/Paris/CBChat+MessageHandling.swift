...
extension CBChat {
    public enum MessageInput: CustomDebugStringConvertible, CustomStringConvertible {
        ...
        public func handle(message: CBMessage?, leaf: CBChatIdentifier) throws -> CBMessage {
            var message = message
            switch self {
            case .item(let item):
                if message != nil {
                    try message!.handle(item: item)
                    return message!
                }
                message = try CBMessage(item: item, chat: leaf)
                return message!
            case .dict(let dict):
                if message != nil {
                    try message!.handle(dictionary: dict)
                    return message!
                }
                message = try CBMessage(dictionary: dict, chat: leaf)
                return message!
            }
        }
    ...
    @discardableResult public func handle(leaf: CBChatIdentifier, input item: MessageInput) throws -> CBMessage? {
        guard let id = item.guid else {
            log.warning("dropping message \(item.debugDescription) as it has an invalid guid?!")
            return nil
        }
        let handledMessage = try item.handle(message: messages[id], leaf: leaf)
    ...
    @discardableResult public func handle(
        leaf: CBChatIdentifier,
        item dictionary: [AnyHashable: Any]
    ) throws -> CBMessage? {
        try handle(leaf: leaf, input: .dict(dictionary))
    }
}

extension CBChat {
    @discardableResult public func handle(leaf: CBChatIdentifier, item: IMItem) throws -> CBMessage? {
        try handle(leaf: leaf, input: .item(item))
    }
}
