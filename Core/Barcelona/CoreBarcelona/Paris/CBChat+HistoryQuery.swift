...
extension CBChat {
    public func rawHistoryQuery(
        ...
        let guids = try await ERResolveGUIDsForChats(
        ...
        var messageIDMapping: [String: [String]] = [:]
        var messageIDs: Set<String> = Set()
        for (messageID, chatID) in guids {
            messageIDMapping[messageID, default: []].append(chatID)
            messageIDs.insert(messageID)
        }
        return BLLoadIMMessageItems(withGUIDs: Array(messageIDs))
            .flatMap { message in
                messageIDMapping[message.id, default: []]
                    .map { chatID in
