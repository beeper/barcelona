...
extension NSMutableAttributedString {
    fileprivate func addAttributes(_ attributes: [TextPartAttribute]? = nil) {
        attributes?
            .forEach {
...
func ERAttributedString(from parts: [MessagePart]) -> MessagePartParseResult {
    ...
    return MessagePartParseResult(
        string: ERInsertMessageParts(
            into: strings.reduce(into: NSMutableAttributedString()) { (accumulator, current) in
...
private func ERAttributedString(from part: MessagePart) -> MessagePartParseResult {
    ...
    case .text:
        return MessagePartParseResult(
            string: ERAttributedString(forText: part.details as NSString, withAttributes: part.attributes),
            transferGUIDs: []
        )
    case .breadcrumb:
        return MessagePartParseResult(
            string: ERAttributedString(forBreadcrumbAttributes: part.attributes ?? []),
            transferGUIDs: []
        )
    case .attachment:
...
private func ERAttributedString(
    forText text: NSString,
    withAttributes attributes: [TextPartAttribute]? = nil
) -> NSAttributedString {
