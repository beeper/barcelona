...
extension BLRegressionTesting {
    /// Change me before running
    public static var validMailtoURI: String = "mailto:info@orders.apple.com"
    /// Change me before running
    public static var validTelURI: String = "tel:+15555555555"

    public static var uris: [String] {
        [validMailtoURI, validTelURI]
    }

    public static var unprefixedURIs: [String] {
        uris.map(\.fastDroppingURIPrefix)
    }

    public static var handles: [IMHandle] {
        IMAccountController.shared.activeAccounts!.filter { $0.service?.id != .FaceTime }
            .flatMap { account in
                                ...
                                do {
                                    ...
                                    for (chat, message) in chats {
                                        ...
                                        if iMessage.imChat == nil && sms.imChat == nil {
                                            preconditionFailure(
                                                "Failed to resolve IMChat[\(chat.scheme):\(chat.value)] for message \(message.id)"
                                            )
                                        }
                                    }

                                    print(
                                        ">>> Raw history query for \(chat.chatIdentifiers.joined(separator: ",")) completed"
                                    )
                                } catch {
                                    preconditionFailure(
                                        "Failed to do history query on on chat \(chat.IMChats.map(\.debugDescription)): \(error.localizedDescription)"
                                    )
                                }
        ...
        func go(_ outcome: Outcome) {
            ...
            if outcome == .succeed {
                guard chat.forceRefresh else {
                    preconditionFailure(
                        "SMS IMChat pointing to an email should have requested refresh, but it did not."
                    )
                }
                chat.refreshServiceForSendingIfNeeded()
                guard chat.account.service?.id == .iMessage else {
                    preconditionFailure(
                        "SMS IMChat pointing to an email should have retargeted to iMessage, but it did not."
                    )
                }
            } else {
            ...
            let message = IMMessage(
                sender: nil,
                time: nil,
                text: NSAttributedString(string: "asdf"),
            ...
            if outcome == .succeed {
                guard item.serviceStyle == .iMessage else {
                    preconditionFailure(
                        "SMS IMChat pointing to an email should have retargeted to iMessage, but the message sent over \(item.service ?? "SMS")"
                    )
                }
            } else {
                guard item.serviceStyle == .SMS else {
                    preconditionFailure(
                        "SMS IMChat pointing to an email should have sent the message over SMS during repro, but the message sent over \(item.service ?? "iMessage")"
                    )
                }
            }
        }
