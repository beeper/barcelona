...
extension IMAccount {
    fileprivate var registrationSuccessDate: Date? {
        dictionary["RegistrationSuccessDate"] as? Date
    }

    fileprivate var registrationFailureDate: Date? {
...
extension IMAccountController {
    fileprivate var state: BridgeState {
        ...
        if [
            IMAccountRegistrationFailureReason.irreparableFailure, .expiredDeviceCredentials, .loginFailed,
            .badCredentials, .badDeviceCredentials, .badPushToken, .cancelled,
        ]
        .contains(account.registrationFailureReason) {
    ...
    fileprivate var registrationFailureAlertInfo: [String: Any?] {
...
extension BridgeStatusCommand {
    public static var current: BridgeStatusCommand {
